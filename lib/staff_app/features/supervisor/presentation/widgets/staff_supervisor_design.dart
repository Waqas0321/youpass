import 'package:flutter/material.dart';

abstract final class StaffSupervisorDesign {
  static const accent = Color(0xFFD4A044);
  static const pageBackground = Color(0xFFF8F9FA);
  static const successGreen = Color(0xFF22C55E);
  static const successBackground = Color(0xFFECFDF5);
  static const dangerRed = Color(0xFFDC2626);
  static const dangerBackground = Color(0xFFFFF1F2);
  static const warningBackground = Color(0xFFFFF8EB);
  static const tileBackground = Color(0xFFFFF8EB);
  static const tileBorder = Color(0xFFFDE6B0);
}
